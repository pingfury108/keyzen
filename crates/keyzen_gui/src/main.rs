...
impl KeyzenApp {
    ...
    fn render_practice_area(&self, session: &SessionModel) -> Div {
        ...
        div()
            ...
            .when(snapshot.pinyin_state.is_some(), |parent| {
                // 拼音候选框
                if let Some(pinyin_state) = &snapshot.pinyin_state {
                    if pinyin_state.composing {
                        parent.child(
                            div()
                                .w_full()
                                .p_6()
                                            ...
                                            div()
                                                ...
                                                .child(
                                                    div()
                                                        ...
                                                        .font_weight(FontWeight::BOLD)
                                                        .text_color(rgb(0x00C2B8))
                                                        .child(pinyin_state.buffer.clone()),
                                                ),
                                        )
                                        .when(!pinyin_state.candidates.is_empty(), |parent| {
                                            parent.child(
                                                // 候选字列表
                                                div()
                                                    .flex()
